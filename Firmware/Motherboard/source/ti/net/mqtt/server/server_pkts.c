...
//
//*****************************************************************************
static bool accept_ctx(int32_t net, uint32_t waitSecs)
{
...
//
//*****************************************************************************
static bool proc_recv_hnds_locked(int32_t *recv_hnds, int32_t n_hnds, uint32_t waitSecs)
{
    bool rv = true;
    int32_t idx = 0;
    int32_t net;
    ...
    for (idx = 0; (idx < n_hnds) && (rv == true); idx++)
    {
        ...
        {
            if (proc_loopback_recv(MQTTServerPkts_loopbNet) < 0)
            {
                rv = false;
            }
        }
...
static int32_t net_hnds_mon_loop(uint32_t waitSecs)
{
    ...
    do
    {
        ...
        if (false == proc_recv_hnds_locked(MQTTServerPkts_recvHvec, n_hnds, waitSecs))
        {
            return MQTT_PACKET_ERR_LIBQUIT;
        }
    } while (1);
