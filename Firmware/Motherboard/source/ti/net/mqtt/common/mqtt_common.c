...
{
    MQTT_bufSet((uint8_t*) cq->idVec, 0, sizeof(cq->idVec));
...
bool MQTT_qos2PubCqLogup(MQTT_PubQOS2CQ_t *cq, uint16_t msgID)
{
    if (0 != cq->nFree)
    {
        cq->idVec[cq->wrIdx++] = msgID;
        cq->wrIdx &= MQTT_MAX_PUBREL_INFLT - 1;
        cq->nFree--;
        return true;
    }
...
bool MQTT_qos2PubCqUnlog(MQTT_PubQOS2CQ_t *cq, uint16_t msgID)
{
    ...
    if (cq->nFree < MQTT_MAX_PUBREL_INFLT)
    {
         ...
         */
        if (msgID == cq->idVec[cq->rdIdx++])
        {
            cq->rdIdx &= MQTT_MAX_PUBREL_INFLT - 1;
            cq->nFree++;
            return true;
        }
    }
...
bool MQTT_qos2PubCqCheck(MQTT_PubQOS2CQ_t *cq, uint16_t msgID)
{
    ...
    {
        if (msgID == cq->idVec[rdIdx++])
        {
            return true;
        }
        rdIdx &= MQTT_MAX_PUBREL_INFLT - 1;
    }
